        ...
        <>
            {spot && (
                <div className='spot-update-container'>
                    <div className='spot-update-form'>
                        <div className='form-header'>
                            <h1 className='form-header-text'>Update your Spot</h1>
                            <p className='form-header-subtext'>Edit your place details</p>
                        </div>
                        <form onSubmit={handleSubmit}>
                            <div className='spot-country'>
                                <label className='spot-label'>
                                    <div className='error-div'>
                                        ...
                                        {!errors.country && <p className='normal-text'>Country</p>}
                                    </div>
                                    <input type="text" placeholder='Country' value={spot && country} onChange={e => setCountry(e.target.value)} />
                                </label>
                            </div>
                            <div className='spot-street'>
                                <label className='spot-label'>
                                    <div className='error-div'>
                                        {errors.address && <p className='error-text'>{errors.address}</p>}
                                        {!errors.address && <p className='normal-text'>Street Address</p>}
                                    </div>
                                    <input type='text' placeholder='Address' value={spot && address} onChange={e => setAddress(e.target.value)} />
                                </label>
                            </div>
                            <div className='spot-city-state'>
                                <div className='spot-label'>
                                    <div className='error-div'>
                                        ...
                                        {!errors.city && <p className='normal-text'>City</p>}
                                    </div>
                                    <input className='input-exc' type='text' placeholder='City' value={spot && city} onChange={e => setCity(e.target.value)} />
                                </div>
                                <div className='spot-label'>
                                    <div className='error-div'>
                                        {errors.state && <p className='error-text'>{errors.state}</p>}
                                        {!errors.state && <p className='normal-text'>State</p>}
                                    </div>
                                    <input type='text' className='input-exc' placeholder='State' value={spot && state} onChange={e => setState(e.target.value)} />
                                </div>
                            </div>
                            <hr />
                            <p className='secondary-text'>Describe your place to guests</p>
                            <p className='tertiary-text'>Mention the best features of your space, any special amenities like fast wifi or parking, and what you love about the neighborhood</p>
                            <textarea placeholder='Please write at least 30 characters' value={spot && description} onChange={e => setDescription(e.target.value)} />
                            <div className='error-div'>
                                {errors.description && <p className='error-text'>{errors.description}</p>}
                            </div>
                            <hr />
                            <p className='secondary-text'>Create a title for your spot</p>
                            <p className='tertiary-text'>Catch guests' attention with a spot title that highlights what makes your place special.</p>
                            <input type='text' placeholder='Name of your spot' value={spot && name} onChange={e => setName(e.target.value)} />
                            <div className='error-div'>
